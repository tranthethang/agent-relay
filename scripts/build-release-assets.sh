...
cp "$ROOT_DIR/install.sh" "$TARGET_STAGE/"
cp "$ROOT_DIR/uninstall.sh" "$TARGET_STAGE/"
cp "$ROOT_DIR/verify.sh" "$TARGET_STAGE/"
...
for script in install.sh uninstall.sh verify.sh; do
  ...
  sed "s/^DEFAULT_REF=\"\"/DEFAULT_REF=\"$TAG\"/" "$ROOT_DIR/$script" > "$dest"
  chmod +x "$dest"

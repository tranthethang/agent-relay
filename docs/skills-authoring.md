...
5. **Parallel mode** — if `implement-plan/` (or legacy `implement-plan-<id>/`)
   exists, require `task-claim.sh` for status/report writes; forbid hand-editing
   rollups; point at `check`. See [task-claim.md](task-claim.md).  
